        ...
        methods: {
            ...
            setChartData(){
                ...
                for (const item of this.dataSet) {
                    data[item.stdt] = Math.floor(item.move / 360);
                }
                this.chartData = {
                    labels,
                    datasets: [
            ...
            loadData(type) {
                ...
                let enDate = curDate;
                if (this.chartView === 0) {
                    query = "SELECT SUM(move) as move, (stdt-?)/3600 stdt FROM logs_v2 WHERE cat = ? and stdt >= ? AND stdt < ? GROUP BY (stdt-?)/3600";
                    stDate = curDate.clone();
                    enDate = curDate.clone().date(curDate.date()+1);
                } else if (this.chartView === 1) {
                    query = "SELECT SUM(move) as move, (stdt-?)/86400 stdt FROM logs_v2 WHERE cat = ? and stdt >= ? AND stdt < ? GROUP BY (stdt-?)/86400";
